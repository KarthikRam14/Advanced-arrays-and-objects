...
const letter = strings.filter(function(aboveFiveChar){
  return aboveFiveChar.length>=5;
});
console.log(letter)
...
const conc = alphabets.reduce(function(first, item){
  return first + item;
});
console.log(conc);
...
const firstEven = number.find(function(item){
  return item%2 == 0;
});
...
const indexOfSeven = ranks.findIndex(function(element){
  return element == 7;
});
...
//3:8
const num = [2, 4, 6, 8];
const each = num.forEach(function(item, index){
...
const atLeast = marks.some(function(item){
  return item<45;
});
console.log(atLeast)
...
const check = positive.every(function(item){
  return item>0;
});
console.log(check)
...
console.log(person.vehicles.hasOwnProperty('house'))
